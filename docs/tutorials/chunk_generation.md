...
Don't worry! Go to [Lightning Fast Generation](optimization.md) for a quick tutorial on how to improve speeds.

## 3D Chunk Generation

Chunk loading in 3D works the same as in 2D. Nodes mentioned above have their own 3D variant, such as `ChunkLoader3D`. Tips for optimization can also be found in [Lightning Fast Generation](optimization.md). In 3D, these are especially important as it requires more power.s